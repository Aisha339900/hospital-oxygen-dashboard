...
With the backend running, the dashboard loads measurements, history, alarms, and system health from the API (see `frontend/src/utils/liveDashboardMapper.js`). If the API is unreachable or empty, the app falls back to client-side mock generators in [frontend/src/utils/mockGenerators.js](frontend/src/utils/mockGenerators.js). UI copy and chart metadata live under `frontend/src/config/`.
...
Edit the threshold logic returned by `generateStreamStatus` inside [frontend/src/utils/mockGenerators.js](frontend/src/utils/mockGenerators.js):
...
Customize the offline mock telemetry by editing `generateTrendSeries` and related helpers in [frontend/src/utils/mockGenerators.js](frontend/src/utils/mockGenerators.js).
